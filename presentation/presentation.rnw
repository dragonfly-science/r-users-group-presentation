...
\begin{frame}
  \frametitle{Programming VS. hand drawing}
    ...
    \item Transparent
    \item Insurance of correctness
      \begin{itemize}
      \item No hand tweaking of e.g. bubble sizes or region colour
      \item What you see is directly from the original data
      \end{itemize}
    \end{itemize}
\end{frame}
...
Simple plots in R are quick and easy, but ugly... %%\\ \vspace{-1cm}
...
Large improvement thanks to \textbf{ggplot2} from Hadley Wickham, \\
but it voluntarily lacks flexibility.
<<eval=T, fig.width=6, fig.height=3, out.width='0.7\\textwidth'>>=
library(ggplot2)
...
\begin{frame}
  ...
  \begin{itemize}
  \item A specific style or design is required (client, journal, organisation...)
...
\begin{frame}
  ...
  \textbf{https://github.com/dragonfly-science/r-users-group-presentation.git}
  
  \vspace{1cm}
  
  Thank you to the whole R and open-source community to make this possible.
